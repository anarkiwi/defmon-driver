...
from vice_driver.binmon import RELEASE_NONE
from vice_driver.vice_docker import ViceContainerError

from ._smoke_support import section, smoke_session
from .defmon import DefmonError
...
def run(d64_path: Path, port: int, keep_container: bool) -> int:
    ...
    # smoke_session copies the pristine d64 into a temp workdir and
    # mounts it RW on drive 8. Don't introduce a separate drive 9 —
    # defMON saves to whatever drive is currently selected (8 by
    # default), so anything we do in the disk menu writes back to this
    # temp image. cleanup_workdir=False keeps the file around for the
    # post-stop c1541 inspection below.
    work_d64: Path | None = None
    save_name = "TUNE1"
    ...
    try:
        with smoke_session(
            ...
            stop_container=not keep_container,
        ) as s:
            d = s.d
            work_d64 = s.workdir / "disk.d64"
            section("CONTAINER STARTED", char="=")
            print(f"  id    : {s.container.container_id}")
            print(f"  binmon: 127.0.0.1:{port}")

            section("WAITING FOR defMON BOOT", char="=")
            print(s.boot_snapshot.text())
    ...
    section("DISK CONTENTS AFTER STOP (check for saved file)", char="=")
    if work_d64 is None:
        print("  (smoke_session never produced a writable disk; skipping)")
    else:
        try:
